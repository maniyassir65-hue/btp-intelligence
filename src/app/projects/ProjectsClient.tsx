...
export default function ProjectsClient({ initialProjects }: { initialProjects: Project[] }) {
    ...
    async function handleAddProject(e: React.FormEvent) {
        ...
        try {
            ...
            formData.append('client_name', newClient);
            formData.append('city', newCity);
            formData.append('global_budget', newBudget);

            const newProject = await addProjectAction(formData);
            setProjects([newProject as any, ...projects]);
            setIsModalOpen(false);
            setNewName(''); setNewCity(''); setNewClient(''); setNewBudget('');
            router.refresh();
        } catch (error: any) {
                                    ...
                                    <div className="p-6 space-y-6 -mt-5 bg-white rounded-t-[1.5rem] relative">
                                        ...
                                        <div className="grid grid-cols-2 gap-4 py-4 border-y border-stone-50">
                                            <div className="space-y-1">
                                                ...
                                                <p className="font-black text-xs text-stone-900 leading-none">
                                                    {(project.global_budget || 0).toLocaleString()} <span className="text-stone-400 font-bold">DH</span>
                                                </p>
                                            </div>
                                        ...
                                        <div className="flex items-center justify-between pt-1">
                                            ...
                                            <div className="flex -space-x-1.5">
                                                <div className="w-7 h-7 rounded-full bg-stone-50 border-2 border-white flex items-center justify-center shadow-inner">
                                                    <span className="text-[9px] font-bold text-stone-300">B</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <form onSubmit={handleAddProject} className="space-y-6">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <input required type="number" value={newBudget} onChange={(e) => setNewBudget(e.target.value)} placeholder="850000"
